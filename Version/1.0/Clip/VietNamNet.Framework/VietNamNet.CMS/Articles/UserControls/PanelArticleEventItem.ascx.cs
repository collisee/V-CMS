...
{
    public partial class PanelArticleEventItem : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (EditOption == 0)
                {
                    ...
                    radGridSelected.ClientSettings.AllowRowsDragDrop = false;
                    radGridSelected.ClientSettings.ClientEvents.OnRowDropping = string.Empty;
                    radGridSelected.ClientSettings.ClientEvents.OnRowDblClick = string.Empty;
                }
                else
                {
            ...
            if (Nulls.IsNullOrEmpty(e.HtmlElement))
            {
                if (e.DraggedItems[0].OwnerGridID == radGridDefault.ClientID)
                {
                        ...
                        || (e.DestDataItem != null && e.DestDataItem.OwnerGridID == radGridSelected.ClientID))
                    {
                        ...
                        {
                            int rid = Utilities.ParseInt(row[CMSConstants.Entities.ArticleEventItem.FieldName.Id]);
                            if (rid <= id) id = rid - 1;
                        }
                        ...
                        foreach (GridDataItem draggedItem in e.DraggedItems)
                        {
                            ...
                            {
                                if (
                                    Utilities.ParseInt(row[CMSConstants.Entities.ArticleEventItem.FieldName.ArticleId]) ==
                                    articleId)
                                {
                                    //change Status
                                    if (
                                        Utilities.StringCompare(
                                            row[CMSConstants.Entities.ArticleEventItem.FieldName.SaveStatus],
                                            Constants.CommonStatus.DELETE))
                                    {
                                        int itemId =
                                            Utilities.ParseInt(row[CMSConstants.Entities.ArticleEventItem.FieldName.Id]);
                                        ...
                                        row[CMSConstants.Entities.ArticleEventItem.FieldName.Last_Modified_At] =
                                            DateTime.Now;
                                        row[CMSConstants.Entities.ArticleEventItem.FieldName.Last_Modified_By] =
                            ...
                            drNew[CMSConstants.Entities.ArticleEventItem.FieldName.SaveStatus] =
                                Constants.CommonStatus.NEW;
                        ...
                        for (int i = 0; i < dtItems.Rows.Count; i++)
                        {
                            ...
                            dtItems.Rows[i][CMSConstants.Entities.ArticleEventItem.FieldName.Last_Modified_At] =
                                DateTime.Now;
                            dtItems.Rows[i][CMSConstants.Entities.ArticleEventItem.FieldName.Last_Modified_By] =
                                Utilities.ParseInt(Session[Constants.Session.USER_ID]);
                        }
        ...
        protected void radGridDefault_ItemCommand(object source, GridCommandEventArgs e)
        {
                 ...
                 Utilities.StringCompare(e.CommandName, Constants.CommandNames.Edit)) && e.Item is GridDataItem)
            {
                ...
                if (!blnExist)
                {
                    ...
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.Id] = id;
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.Ord] = dtItems.Rows.Count + 1;
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.ArticleEventId] = 0;
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.ArticleId] = articleId;
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.ArticleName] = articleName;
                    drNew[CMSConstants.Entities.ArticleEventItem.FieldName.ArticleAuthor] = string.Empty;
        ...
        {
            if (radDpkPublishFromDate.SelectedDate != null)
            {
                return (DateTime)radDpkPublishFromDate.SelectedDate;
            }
        ...
        {
            if (radDpkPublishToDate.SelectedDate != null)
            {
                return (DateTime)radDpkPublishToDate.SelectedDate;
            }
