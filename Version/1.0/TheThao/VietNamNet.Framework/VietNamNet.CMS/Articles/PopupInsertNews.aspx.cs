...
{
    public partial class PopupInsertNews : ArticleBasePageView
    {
        ...
        protected override DataTable GetDataSource()
        {
            if (GetCategoryId() == 0)
            {
                ActionName = CMSConstants.Services.ArticleManager.Actions.ViewGetAllArticleForPopupInsertNewsWithoutCategory;
            }
        ...
        protected override void radGridDefault_ItemCommand(object source, GridCommandEventArgs e)
        {
                ...
                Utilities.StringCompare(e.CommandName, Constants.CommandNames.Edit)) && e.Item is GridDataItem)
            {
                ...
                string url = e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["Url"].ToString();
                ...
                if (!blnExist)
                {
                    ...
                    drNew["Url"] = url;
                    drNew["Link"] = Utilities.GetArticleUrl(url, articleId, articleName);
                ...
                if (e.DraggedItems[0].OwnerGridID == radGridDefault.ClientID)
                {
                        ...
                        || (e.DestDataItem != null && e.DestDataItem.OwnerGridID == radGridSelected.ClientID))
                    {
                        ...
                        foreach (GridDataItem draggedItem in e.DraggedItems)
                        {
                            ...
                            string url = draggedItem.GetDataKeyValue("Url").ToString();
