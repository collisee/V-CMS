...
{
    /// <summary>
    /// Summary description for CollaboratorManager.
    /// </summary>
    public class CollaboratorManager : Facade
    {
        ...
        public override Packet Execute(Packet param)
        {
            switch (param.Action)
            {
                ...
                case CMSConstants.Services.CommonActions.LISTALL:
                    ListAll(param);
                    break;
                case CMSConstants.Services.CollaboratorManager.Actions.ViewGetAllCollaborator:
                    ViewGetAllCollaborator(param);
                    break;
                default:
                    break;
            }
        ...
        {
            if (Utilities.ParseInt(param.RawData.Tables[0].Rows[0][CMSConstants.Entities.Collaborator.FieldName.Id]) == 0)
            {
                CollaboratorDAO.Insert(param.RawData.Tables[0].Rows[0]);
            }
            else
            {
                CollaboratorDAO.Update(param.RawData.Tables[0].Rows[0]);
            }
        }
        ...
        {
            CollaboratorDAO.Delete(param.RawData.Tables[0].Rows[0]);
        }
        ...
        {
            DataTable dt = CollaboratorDAO.SelectOne(param.RawData.Tables[0].Rows[0]);
        ...
        {
            DataTable dt = CollaboratorDAO.SelectAll();
            param.RawData.Tables.Clear();
            param.RawData.Tables.Add(dt.Copy());
        }
        ...
        #region Function ViewGetAllCollaborator

        private void ViewGetAllCollaborator(Packet param)
        {
            param.RawData = CollaboratorDAO.ViewGetAllCollaborator(param.RawData);
        }
