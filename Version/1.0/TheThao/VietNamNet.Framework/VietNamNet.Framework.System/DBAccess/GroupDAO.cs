...
namespace VietNamNet.Framework.System.DBAccess
{
    public class GroupDAO : BaseDAO
    {
        ...
        {
            string insertString = "Insert Into [Group]" +
            ...
            {
                throw new Exception("GroupDAO::Insert::Error", ex);
            }
        }
        ...
        {
            string updateString = @"Update [Group] Set" +
            ...
            {
                throw new Exception("GroupDAO::Update::Error", ex);
            }
        }
        ...
        {
            string deleteString =
                @"UPDATE [Group] SET [Last_Modified_At] = @Last_Modified_At, [Last_Modified_By] = @Last_Modified_By, [flag] = 'D' WHERE [Id] = @Id";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("GroupDAO::Delete::Error", ex);
            }
        }
        ...
        {
            string selectString = @"Select * From [Group] Where [Id] = @Id AND (ISNULL(flag, '') = '')";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("GroupDAO::SelectOne::Error", ex);
            }
        }
        ...
        {
            string selectAllString = "Select * From [Group] Where (ISNULL(flag, '') = '') ORDER BY [Name]";
            try
            {
            ...
            {
                throw new Exception("GroupDAO::SelectAll::Error", ex);
            }
        }

        #region ViewGetAllGroup

        public static DataSet ViewGetAllGroup(DataSet dataSet)
        {
            return ViewGetAllGroup(null, dataSet);
        }

        public static DataSet ViewGetAllGroup(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToCountRecord = "SELECT COUNT(*) "
                                           ...
                                           CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          SystemConstants.Entities.Group.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          SystemConstants.Entities.Group.Name);

            //generate query string
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         + "SELECT *, row_number() over(order by [Name]) RowNum FROM [Group] "
                                         + CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          SystemConstants.Entities.Group.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          SystemConstants.Entities.Group.Name)
