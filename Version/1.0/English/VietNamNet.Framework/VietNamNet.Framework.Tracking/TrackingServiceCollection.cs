...
{
    [Serializable()]
    public class TrackingServiceCollection : CollectionBase
    {
        private SortedList _this = new SortedList();

        public TrackingServiceObject this[int index]
        {
            get { return (TrackingServiceObject)_this[index]; }
        ...
        public virtual void Add(TrackingServiceObject obj)
        {
            ...
            foreach (string action in obj.Actions.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                else
                {
                    if (!_this.Contains(key))
                    {
                        ...
                        throw new Exception(string.Format("Tracking Configuration - Service: The name {0} is duplicated.", key));
                }
            }
        }
    }
