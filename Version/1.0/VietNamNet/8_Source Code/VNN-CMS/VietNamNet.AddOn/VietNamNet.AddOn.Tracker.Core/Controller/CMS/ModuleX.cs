...
namespace VietNamNet.AddOn.Tracker.Core.CMS
{
	...
	public partial class ModuleX : ActiveRecord<ModuleX>, IActiveRecord
	{
		...
		private static void GetTableSchema() 
		{
			...
			{
				//Schema declaration
				TableSchema.Table schema = new TableSchema.Table("Module", TableType.Table, DataService.GetInstance("VNN"));
				...
				DataService.Providers["VNN"].AddSchema("Module",schema);
			}
		}
		#endregion
		...
		public VietNamNet.AddOn.Tracker.Core.CMS.PolicyCollection PolicyRecords()
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.PolicyCollection().Where(Policy.Columns.ModuleId, Id).Load();
		}
		public VietNamNet.AddOn.Tracker.Core.CMS.MenuCollection MenuRecords()
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.MenuCollection().Where(Menu.Columns.ModuleId, Id).Load();
		}
		public VietNamNet.AddOn.Tracker.Core.CMS.FunctionCollection FunctionRecords()
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.FunctionCollection().Where(Function.Columns.ModuleId, Id).Load();
		}
		#endregion
