...
namespace VietNamNet.Framework.System.DBAccess
{
    public class ModuleDAO : BaseDAO
    {
        ...
        {
            string insertString = "Insert Into [Module]" +
            ...
            {
                throw new Exception("ModuleDAO::Insert::Error", ex);
            }
        }
        ...
        {
            string updateString = @"Update [Module] Set" +
            ...
            {
                throw new Exception("ModuleDAO::Update::Error", ex);
            }
        }
        ...
        {
            string deleteString =
                @"UPDATE [Module] SET [Last_Modified_At] = @Last_Modified_At, [Last_Modified_By] = @Last_Modified_By, [flag] = 'D' WHERE [Id] = @Id";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("ModuleDAO::Delete::Error", ex);
            }
        }
        ...
        {
            string selectString = @"Select * From [Module] Where [Id] = @Id AND (ISNULL(flag, '') = '')";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("ModuleDAO::SelectOne::Error", ex);
            }
        }

        public static DataTable GetModuleByAlias(DataRow dataRow)
        {
            return GetModuleByAlias(null, dataRow);
        }

        public static DataTable GetModuleByAlias(SqlTransaction sqlTransaction, DataRow dataRow)
        {
            string selectString = @"SELECT * FROM [Module] WHERE [Id] <> @Id AND [Alias] = @Alias AND (ISNULL(flag, '') = '')";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("ModuleDAO::GetModuleByAlias::Error", ex);
            }
        }

        #region ViewGetAllModule

        public static DataSet ViewGetAllModule(DataSet dataSet)
        {
            return ViewGetAllModule(null, dataSet);
        }

        public static DataSet ViewGetAllModule(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToCountRecord = "SELECT COUNT(*) "
                                           ...
                                           CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          SystemConstants.Entities.Module.SearchColumns,
            ...
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         + "SELECT *, row_number() over(order by [Name]) RowNum FROM [Module] "
                                         + CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          SystemConstants.Entities.Module.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          SystemConstants.Entities.Module.Name)
        ...
        {
            string selectAllString = "Select * From [Module] Where (ISNULL(flag, '') = '') ORDER BY [Name]";
            try
            {
            ...
            {
                throw new Exception("ModuleDAO::SelectAll::Error", ex);
            }
        }

        public static DataTable GetTemplateTable()
        {
            DataTable table = new DataTable("Module");
