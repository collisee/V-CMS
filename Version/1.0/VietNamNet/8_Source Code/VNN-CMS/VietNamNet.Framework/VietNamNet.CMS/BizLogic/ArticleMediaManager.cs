...
{
    /// <summary>
    /// Summary description for ArticleMediaManager.
    /// </summary>
    public class ArticleMediaManager : Facade
    {
        ...
        {
            if (Utilities.ParseInt(param.RawData.Tables[0].Rows[0][CMSConstants.Entities.ArticleMedia.FieldName.Id]) == 0)
            {
                ArticleMediaDAO.Insert(param.RawData.Tables[0].Rows[0]);
            }
            else
            {
                ArticleMediaDAO.Update(param.RawData.Tables[0].Rows[0]);
            }
        }
        ...
        {
            ArticleMediaDAO.Delete(param.RawData.Tables[0].Rows[0]);
        }
        ...
        {
            DataTable dt = ArticleMediaDAO.SelectOne(param.RawData.Tables[0].Rows[0]);
        ...
        {
            DataTable dt = ArticleMediaDAO.SelectAll();
            param.RawData.Tables.Clear();
            param.RawData.Tables.Add(dt.Copy());
        }
