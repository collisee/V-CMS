...
// IE6 fix to hide selects boxes when primary drop downs are visible
// raw JS version, no jQuery or Prototype dependancies
// built specificly for IE6
if (typeof window.XMLHttpRequest == "undefined") {
    window.attachEvent('onload', function() {
        // get all primary nav drop downs
        var primaryNav = document.getElementById('section-nav');
        if (primaryNav != null) {
            // set select visiblilty to hidden when the mouse enters a tab
            primaryNav.attachEvent('onmouseenter', function() {
                var selects = document.getElementsByTagName('select');
                for(var j=0; j<selects.length; j++) {
                    selects[j].style.visibility = 'hidden';
                }
            });
            // set select visiblilty to visible when the mouse leaves the tab
            primaryNav.attachEvent('onmouseleave', function() {
                var selects = document.getElementsByTagName('select');
                for(var j=0; j<selects.length; j++) {
                    selects[j].style.visibility = 'visible';
                }
