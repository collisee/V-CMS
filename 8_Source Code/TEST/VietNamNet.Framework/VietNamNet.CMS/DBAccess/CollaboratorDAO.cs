...
namespace VietNamNet.CMS.DBAccess
{
    public class CollaboratorDAO : BaseDAO
    {
        ...
        {
            string insertString = "Insert Into [Collaborator]" +
            ...
            {
                throw new Exception("CollaboratorDAO::Insert::Error", ex);
            }
        }
        ...
        {
            string updateString = @"Update [Collaborator] Set" +
            ...
            {
                throw new Exception("CollaboratorDAO::Update::Error", ex);
            }
        }
        ...
        {
            string deleteString =
                @"UPDATE [Collaborator] SET [Last_Modified_At] = @Last_Modified_At, [Last_Modified_By] = @Last_Modified_By, [flag] = 'D' WHERE [Id] = @Id";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("CollaboratorDAO::Delete::Error", ex);
            }
        }
        ...
        {
            string selectString = @"Select * From [Collaborator] Where [Id] = @Id AND (ISNULL(flag, '') = '')";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("CollaboratorDAO::SelectOne::Error", ex);
            }
        }
        ...
        {
            string selectAllString = "Select * From [Collaborator] Where (ISNULL(flag, '') = '')";
            try
            {
            ...
            {
                throw new Exception("CollaboratorDAO::SelectAll::Error", ex);
            }
        }

        #region ViewGetAllCollaborator

        public static DataSet ViewGetAllCollaborator(DataSet dataSet)
        {
            return ViewGetAllCollaborator(null, dataSet);
        }

        public static DataSet ViewGetAllCollaborator(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToCountRecord = "SELECT COUNT(*) "
                                           ...
                                           CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          CMSConstants.Entities.Collaborator.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          CMSConstants.Entities.Collaborator.Name);

            //generate query string
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         + "SELECT *, row_number() over(order by [Name]) RowNum FROM [Collaborator] "
                                         + CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          CMSConstants.Entities.Collaborator.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          CMSConstants.Entities.Collaborator.Name)
        ...
        {
            DataTable table = new DataTable("Collaborator");
