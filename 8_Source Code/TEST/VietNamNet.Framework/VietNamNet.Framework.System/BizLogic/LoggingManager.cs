...
namespace VietNamNet.Framework.System.BizLogic
{
    ...
    /// </summary>
    public class LoggingManager : Facade
    {
        ...
        public override Packet Execute(Packet param)
        {
            switch (param.Action)
            {
                ...
                case BaseServices.CommonActions.LISTALL:
                    ListAll(param);
                    break;
                case SystemConstants.Services.LoggingManager.Actions.GetLogging:
                    GetLogging(param);
                    break;
                default:
                    break;
        ...
        {
            if (Utilities.ParseInt(param.RawData.Tables[0].Rows[0][SystemConstants.Entities.Logging.FieldName.Id]) == 0)
            {
                LoggingDAO.Insert(param.RawData.Tables[0].Rows[0]);
            }
            else
            {
                LoggingDAO.Update(param.RawData.Tables[0].Rows[0]);
            }
        }
        ...
        {
            LoggingDAO.Delete(param.RawData.Tables[0].Rows[0]);
        }
        ...
        {
            DataTable dt = LoggingDAO.SelectOne(param.RawData.Tables[0].Rows[0]);
        ...
        {
            DataTable dt = LoggingDAO.SelectAll();
            param.RawData.Tables.Clear();
            param.RawData.Tables.Add(dt.Copy());
        }
        ...
        #region Function GetLogging

        private void GetLogging(Packet param)
        {
            DataTable dt = LoggingDAO.GetLogging(param.RawData.Tables[0].Rows[0]);
            param.RawData.Tables.Clear();
            param.RawData.Tables.Add(dt.Copy());
        }
