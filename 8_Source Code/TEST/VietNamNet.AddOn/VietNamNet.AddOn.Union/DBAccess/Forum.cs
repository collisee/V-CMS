...
{
  public class clsDataAccess : BaseDAO
  {
    public clsDataAccess()
    { }
    SqlConnection mycon = new SqlConnection(Utilities.GetConfigKey("SqlConnectionString"));
    ...
    #region AddNewMessage

    public void AddNewMessage(string query)
    {
      string selectString = @"INSERT into " + Utilities.GetConfigKey("CommentTable") + "(ParentId,ArticleId,Title,UserName,UserEmail,Description,Indent,UserProfile,CommentType) VALUES (@mParentId,@mArticleId,@mTitle,@mUserName,@mUserEmail,@mDescription,@mIndent,@mProfile,@mCommentType)";
      try
      {
        //SqlCommand sqlCommand = new SqlCommand();
        //sqlCommand.CommandText = query;
        ...
        SqlParameter[] paramArray = new SqlParameter[]
                    {
                        CreateSqlPrameter("@mParentId", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@mArticleId", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@mTitle", SqlDbType.NVarChar, 255, ParameterDirection.Input),
                        CreateSqlPrameter("@mUserName", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@mUserEmail", SqlDbType.Int, 0, ParameterDirection.Input),                        
                        CreateSqlPrameter("@mDescription", SqlDbType.NVarChar, 255, ParameterDirection.Input),
                        CreateSqlPrameter("@mIndent", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@mProfile", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@mCommentType", SqlDbType.Int, 0, ParameterDirection.Input)
                    };

        ExecuteNonQuery(selectString, paramArray);
      }
      catch (Exception ex)
      {
        throw new Exception("UnionDAO::AddNewMessage::Error", ex);
      }
    }
