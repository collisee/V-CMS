...
{
    public class AdvertisementZoneDAO : BaseDAO
    {
        ...
        {
            string insertString = "Insert Into [AdvertisementZone]" +
                                  "([Name],[HolderId],[Detail],[Mode],[Direction],[Width],[Height],[Created_At],[Created_By],[Last_Modified_At],[Last_Modified_By],[flag]) " +
                                  "Values(@Name, @HolderId, @Detail, @Mode, @Direction, @Width, @Height, @Created_At, @Created_By, @Last_Modified_At, @Last_Modified_By, @Flag) " +
                ...
                SqlParameter[] paramArray = new SqlParameter[]
                    {
                        ...
                        CreateSqlPrameter("@HolderId", SqlDbType.NVarChar, 255, ParameterDirection.Input),
                        CreateSqlPrameter("@Detail", SqlDbType.NVarChar, 4000, ParameterDirection.Input),
                        CreateSqlPrameter("@Mode", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@Direction", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@Width", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@Height", SqlDbType.Int, 0, ParameterDirection.Input),
            ...
            {
                throw new Exception("AdvertisementZoneDAO::Insert::Error", ex);
            }
        }
        ...
        {
            string updateString = @"Update [AdvertisementZone] Set" +
                                  " [Name] = @Name, [HolderId] = @HolderId, [Detail] = @Detail, [Mode] = @Mode, [Direction] = @Direction, [Width] = @Width, [Height] = @Height, [Created_At] = @Created_At, [Created_By] = @Created_By, [Last_Modified_At] = @Last_Modified_At, [Last_Modified_By] = @Last_Modified_By, [flag] = @flag " +
            ...
            {
                throw new Exception("AdvertisementZoneDAO::Update::Error", ex);
            }
        }
        ...
        {
            string deleteString =
                @"UPDATE [AdvertisementZone] SET [Last_Modified_At] = @Last_Modified_At, [Last_Modified_By] = @Last_Modified_By, [flag] = 'D' WHERE [Id] = @Id";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("AdvertisementZoneDAO::Delete::Error", ex);
            }
        }
        ...
        {
            string selectString = @"Select * From [AdvertisementZone] Where [Id] = @Id AND (ISNULL(flag, '') = '')";
            try
            {
                SqlParameter[] paramArray = new SqlParameter[]
                    {
            ...
            {
                throw new Exception("AdvertisementZoneDAO::SelectOne::Error", ex);
            }
        }

        #region ViewGetAllAdvertisementZone

        public static DataSet ViewGetAllAdvertisementZone(DataSet dataSet)
        {
            return ViewGetAllAdvertisementZone(null, dataSet);
        }

        public static DataSet ViewGetAllAdvertisementZone(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            //generate query count record string
            string strQueryToCountRecord = "SELECT COUNT(*) "
                                           + " FROM [AdvertisementZone] "
                                           +
                                           CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          AdvertisementConstants.Entities.AdvertisementZone.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
                                                          AdvertisementConstants.Entities.AdvertisementZone.Name);

            //generate query string
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         + "SELECT *, row_number() over(order by [Name]) RowNum "
                                         + " , CASE [Mode] WHEN 0 THEN N'Ngẫu nhiên' ELSE N'Thứ tự' END [ModeName] "
                                         + " , CASE [Direction] WHEN 0 THEN N'Chiều dọc' ELSE N'Chiều ngang' END [DirectionName] "
                                         + "FROM [AdvertisementZone] "
                                         + CommonDAO.SearchingQuery(dataSet.Tables[Constants.Paging.Direction.SearchingTableName],
                                                          AdvertisementConstants.Entities.AdvertisementZone.SearchColumns,
                                                          ModuleConstants.ConnectorString.WHERE_STRING,
        ...
        {
            string selectAllString = "Select * From [AdvertisementZone] Where (ISNULL(flag, '') = '') ORDER BY [Name]";
            try
            {
            ...
            {
                throw new Exception("AdvertisementZoneDAO::SelectAll::Error", ex);
            }
        }

        public static DataTable GetTemplateTable()
        {
            DataTable table = new DataTable("AdvertisementZone");
            table.Columns.Add("Id");
            table.Columns.Add("Name");
            table.Columns.Add("HolderId");
