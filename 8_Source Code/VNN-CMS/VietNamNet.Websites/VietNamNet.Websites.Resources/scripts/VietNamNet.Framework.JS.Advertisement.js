        ...
        switch (item.typeAlias){
            ...
            case 'expanding':
                var expanding = '<div> \r\n' + 
                            ...
                            '    wmode="transparent" quality="high" src="' + item.fileLink1 + '" />  \r\n'+
                            ' </div>';
                var expander = '<div style="display:none;position:absolute;"> \r\n' + 
                            '<embed width="' + item.exWidth + '" height="' + item.exHeight + '" align="middle" pluginspage="http://www.macromedia.com/go/getflashplayer"  \r\n'+
                            '    type="application/x-shockwave-flash" allowscriptaccess="sameDomain" name="player"  \r\n'+
                            '    wmode="transparent" quality="high" src="' + item.fileLink2 + '" />  \r\n'+
                            ' </div>';
                if (!item.exMode) { //expand left bottom
                    $(block.holder).append($(expanding).attr('itemId', item.id).attr('itemName', item.name).mouseover(function(){
                        var top = $(this).position().top;
                        var left = $(this).position().left;
                        left = left + block.width - item.exWidth;
                        $(document.body).append($(expander).css({top: top, left: left, width: item.exWidth, height: item.exHeight})
                            .show()
                            .mouseout(function(){
                                $(this).detach();
                            }));
                    }));
                }else{ //expand right bottom
                    $(block.holder).append($(expanding).attr('itemId', item.id).attr('itemName', item.name).mouseover(function(){
                        var top = $(this).position().top;
                        var left = $(this).position().left;
                        $(document.body).append($(expander).css({top: top, left: left, width: item.exWidth, height: item.exHeight})
