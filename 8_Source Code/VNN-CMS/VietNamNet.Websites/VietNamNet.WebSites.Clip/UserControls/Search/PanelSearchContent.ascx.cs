...
namespace VietNamNet.Websites.Clip.UserControls.Search
{
  public partial class PanelSearchContent : UserControl
  {
      ...
      private void PopularItem()
      {
          ...
          if (dt != null && dt.Rows.Count > 0)
          {
              ...
              hplPrev.NavigateUrl = string.Format("/vn/search/index.html?keyword={0}&page={1}", SearchKeyword, PageNumber - 1);
              if (dt.Rows.Count < pageSize) hplNext.Visible = false;
              hplNext.NavigateUrl = string.Format("/vn/search/index.html?keyword={0}&page={1}", SearchKeyword, PageNumber + 1);
          }
          else
          {
              hplPrev.Visible = false; hplNext.Visible = false;
