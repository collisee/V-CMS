    ...
    public class ArticleDAO : BaseDAO
    {
        ...
        public static void Insert(SqlTransaction sqlTransaction, DataRow dataRow)
        {
            string insertString = "Insert Into [Article]" +
                                  "([VersionId],[Status],[ArticleTypeId],[ArticleContentTypeId],[Name],[Title],[Url],[CategoryId],[PartitionId],[SubTitle1],[SubTitle2],[SubTitle3],[SubTitle4],[SubTitle5],[SubTitle6],[ImageLink],[ImageLink1],[ImageLink2],[ImageLink3],[ImageLink4],[ImageLink5],[Lead],[Detail],[PublishDate],[ShowComment],[AuthorId],[Author],[AuthorInfo],[IsMember],[TotalView]," +
                                  "[History],[Created_At],[Created_By],[Last_Modified_At],[Last_Modified_By],[flag]) " +
                                  "Values(@VersionId, @Status, @ArticleTypeId, @ArticleContentTypeId, @Name, @Title, @Url, @CategoryId, @PartitionId, @SubTitle1, @SubTitle2, @SubTitle3, @SubTitle4, @SubTitle5, @SubTitle6, @ImageLink, @ImageLink1, @ImageLink2, @ImageLink3, @ImageLink4, @ImageLink5, @Lead, @Detail, @PublishDate, @ShowComment, @AuthorId, @Author, @AuthorInfo, @IsMember, @TotalView, " +
                                  "@History, @Created_At, @Created_By, @Last_Modified_At, @Last_Modified_By, @Flag) " +
                ...
                SqlParameter[] paramArray = new SqlParameter[]
                    {
                        ...
                        CreateSqlPrameter("@ShowComment", SqlDbType.Int, 0, ParameterDirection.Input),
                        CreateSqlPrameter("@AuthorId", SqlDbType.Int, 0, ParameterDirection.Input),
        ...
        public static void Update(SqlTransaction sqlTransaction, DataRow dataRow)
        {
            string updateString = @"Update [Article] Set" +
                                  ...
                                  " [ImageLink1] = @ImageLink1, [ImageLink2] = @ImageLink2, [ImageLink3] = @ImageLink3, [ImageLink4] = @ImageLink4, [ImageLink5] = @ImageLink5, [Lead] = @Lead, [Detail] = @Detail, [PublishDate] = @PublishDate, [ShowComment] = @ShowComment, [AuthorId] = @AuthorId, [Author] = @Author, [AuthorInfo] = @AuthorInfo, [IsMember] = @IsMember, " +
        ...
        public static DataSet ViewGetAllArticle(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         ...
                                         + "    , [Url] "
                                         + "    , row_number() over(order by [Id] DESC) RowNum FROM [Article] "
                                         + " WHERE (ISNULL(@Status, '') = '' OR [Status] = @Status) "
                                         + " AND (@UId = 0  OR [Created_By] = @UId) "
                                         + " AND DATEDIFF(DD, [Article].[PublishDate], @PublishFromDate) <= 0 "
                                         + " AND DATEDIFF(DD, [Article].[PublishDate], @PublishToDate) >= 0 "
        ...
        public static DataSet ViewGetAllArticleByCategory(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         ...
                                         + "    , [Article].[Url] "
                                         + "    , row_number() over(order by [Article].[PublishDate] DESC, [Article].[Id] DESC) RowNum "
                                         + " FROM [Article] "
                                         + " WHERE [CategoryId] = @CategoryId "
                                         + " AND DATEDIFF(DD, [PublishDate], @PublishFromDate) <= 0 "
                                         + " AND DATEDIFF(DD, [PublishDate], @PublishToDate) >= 0 "
        ...
        public static DataSet ViewGetAllArticleForPopupInsertNews(SqlTransaction sqlTransaction, DataSet dataSet)
        {
            ...
            string strQueryToGetRecord = "SELECT * FROM ( "
                                         ...
                                         + "    , [Article].[ArticleContentTypeId] "
                                         + "    , [Article].[Url] "
                                         + "    , row_number() over(order by [ArticleCategory].[Ord], [Article].[Id] DESC) RowNum "
                                         + " FROM [Article] "
                                         + " 	LEFT JOIN ( "
                                         + " 		SELECT *  "
                                         + " 		FROM [ArticleCategory] "
        ...
        public static DataTable GetTemplateTable()
        {
            ...
            table.Columns.Add("ShowComment", typeof(int));
