...
namespace VietNamNet.Framework.Common
{
    public class CAS
    {
        ...
        public CAS(string baseCasUrl, Page currentPage, bool alwaysRenew)
        {
            if (currentPage == null)
        ...
        /// <param name="baseCasUrl">eg: "https://login.case.edu/cas/"</param>
        /// <param name="currentPage">usually this.Page or this</param>
        public CAS(string baseCasUrl, Page currentPage) : this(baseCasUrl, currentPage, false)
        {
        }
        ...
        public bool Login(string serviceUrl, bool force)
        {
            string loginUrl = string.Format("{0}?service={1}",
                                            Path.Combine(baseCasUrl, "login"), serviceUrl);
            if (AlwaysRenew)
                loginUrl += "&renew=true";
        ...
        {
            //Add by AnhDT: disable Certificate Policy, only use in development 
        ...
        public static string Authenticate(string baseCasUrl, Page page, bool alwaysRenew, bool useCas2)
        {
            ...
            CAS casp = new CAS(baseCasUrl, page, alwaysRenew);
            if (casp.Login())
            {
                try
                {
        ...
        #region Nested type: ValidateException
