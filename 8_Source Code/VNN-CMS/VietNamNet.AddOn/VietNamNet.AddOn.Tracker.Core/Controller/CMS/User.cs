...
namespace VietNamNet.AddOn.Tracker.Core.CMS
{
	...
	public partial class User : ActiveRecord<User>, IActiveRecord
	{
		...
		private static void GetTableSchema() 
		{
			...
			{
				//Schema declaration
				TableSchema.Table schema = new TableSchema.Table("User", TableType.Table, DataService.GetInstance("VNN"));
				...
				DataService.Providers["VNN"].AddSchema("User",schema);
			}
		}
		#endregion
		...
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.GroupMemberCollection().Where(GroupMember.Columns.UserId, Id).Load();
		}
		#endregion
