...
namespace VietNamNet.AddOn.Tracker.Core.CMS
{
	...
	public partial class Group : ActiveRecord<Group>, IActiveRecord
	{
		...
		private static void GetTableSchema() 
		{
			...
			{
				//Schema declaration
				TableSchema.Table schema = new TableSchema.Table("Group", TableType.Table, DataService.GetInstance("VNN"));
				...
				DataService.Providers["VNN"].AddSchema("Group",schema);
			}
		}
		#endregion
		...
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.CategoryPolicyCollection().Where(CategoryPolicy.Columns.GroupId, Id).Load();
		}
		public VietNamNet.AddOn.Tracker.Core.CMS.PolicyCollection PolicyRecords()
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.PolicyCollection().Where(Policy.Columns.GroupId, Id).Load();
		}
		public VietNamNet.AddOn.Tracker.Core.CMS.GroupMemberCollection GroupMemberRecords()
		{
			return new VietNamNet.AddOn.Tracker.Core.CMS.GroupMemberCollection().Where(GroupMember.Columns.GroupId, Id).Load();
		}
		#endregion
